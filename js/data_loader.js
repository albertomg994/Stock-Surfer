function load_year_stock_values(all_companies_stocks, fake) {
  ...
  if (fake == true) {
    ...
    // load Apple stocks values
    var aapl_path = "../data/year/aapl_year_15_response.json";

    loadJSON(aapl_path, function(response) {
      var aapl_data = JSON.parse(response);
      var aapl_stocks = aapl_data.Elements[0].DataSeries.close.values;
      console.log(aapl_stocks);
      all_companies_stocks[0] = aapl_stocks;
    });

    // load Google stock values
    var googl_path = "../data/year/googl_year_15_response.json";

    loadJSON(googl_path, function(response) {
      var googl_data = JSON.parse(response);
      var googl_stocks = googl_data.Elements[0].DataSeries.close.values;
      console.log(googl_stocks);
      all_companies_stocks[1] = googl_stocks;
    });

    // load Microsoft stock values
    var msft_path = "../data/year/msft_year_15_response.json";

    loadJSON(msft_path, function(response) {
      var msft_data = JSON.parse(response);
      var msft_stocks = msft_data.Elements[0].DataSeries.close.values;
      console.log(msft_stocks);
      all_companies_stocks[2] = msft_stocks;
    });
  }
}
